...
import { orgRoleUnit, orgClass } from '@/apis/org'
...
const onSubmit = async () => {
  modal.loading = true
  const { code, msg } = await orgRoleUnit.saveAuthTreeByRoleId({
    roleId: tree.roleId,
...
const getTreeList = async (id) => {
  modal.loading = true
  const { code, msg, data } = await orgRoleUnit.findAuthTreeByRoleId({ id: tree.roleId })
  modal.loading = false
  if (code === 0) {
    // tree.list = data || []
