import { NInput as V, NSwitch as j, NInputNumber as D, NSelect as F, NButton as B, NSpin as $, NModal as O } from "naive-ui";
import { ref as U, reactive as v, resolveComponent as m, openBlock as x, createBlock as S, withCtx as o, createElementVNode as R, createVNode as t, unref as d, createTextVNode as C, resolveDynamicComponent as P } from "vue";
import { a as L } from "./msg-3a5bbd12.mjs";
...
const z = { class: "p-20" }, A = { class: "text-right mt-20" }, G = {
  ...
  setup(K, { expose: f, emit: I }) {
    const c = {
      ...
      s.loading = !0;
      const { code: e, msg: n, data: r } = await L.get({ id: i });
    ...
    return f({
      ...
      const n = V, r = m("sh-form-item"), _ = j, g = D, k = F, b = m("sh-form"), w = B, N = $, T = O;
      return x(), S(T, {
        show: s.show,
  ...
  __name: "index",
  setup(K) {
    const f = U(null), I = [{ label: "关键字", key: "searchKey" }], c = () => ({
      searchKey: "",
      page: 0,
      limit: 20
    });
    ...
    const s = () => {
      ...
      p.loading = !0, u.page++;
      const { code: e, data: n, msg: r } = await L.list(u);
      e === 0 ? (n && (p.list.push(...n), y.value.setTableData({
        data: p.list
      })), (!n || n.length < 10) && (p.isBottom = !0)) : $message.error(r), p.loading = !1;
    };
    return i(), (e, n) => {
      const r = V, _ = m("sh-form-item"), g = m("sh-search-area"), k = m("sh-data-table"), b = B, w = $, N = m("sh-content-layout");
      return x(), S(N, null, {
        header: o(() => [
          t(g, {
            form: d(u),
